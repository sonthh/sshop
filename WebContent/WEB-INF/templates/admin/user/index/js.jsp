    ...
    <script type="text/javascript">
      $(document).ready(function(){
    	  $('#deleteSelected').click(function() {
	    	  ...
	    	  console.log(checked);
	    	  if (checked.length > 0) {
	    		  var check = confirm('Bạn có muốn xóa ' + checked.length + ' user item');
	    		  if (check == true) {
    		    	  $.ajax({
    		    		  type: "post",
    		    		  url: "<%=request.getContextPath()%>/admin/user/deleteSelected",
    		    		  data: {checked: checked},
    		    		  cache: false,
    		    		  success: function(data){
    		    		    alert('Bạn vừa xóa' + data + ' user');
    	  ...
    	  activeUser = function(id, active) {
      		/* console.log(id);  
      		console.log(active); */
  			$.ajax({
  				url: '${pageContext.request.contextPath }/admin/user/activeUser',
  				type: 'POST',
  				cache: false,
  				data: {
  					id: id,
  				...
  				success: function(response){
  					if (response == 1) {
  						$('#activeUser-' + id).toggleClass('active');
  						active = active == 1 ? 0 : 1;
  						$('#activeUser-' + id).attr("onclick","activeUser(" + id + "," + active + ")");
  					}	
  				},
  				error: function (){
