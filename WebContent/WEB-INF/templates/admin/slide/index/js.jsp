    ...
    <script type="text/javascript">
      $(document).ready(function(){
    	  $('#deleteSelected').click(function() {
	    	  ...
	    	  console.log($("input[name='aRow']:checked"));
	    	  console.log(checked);
	    	  if (checked.length > 0) {
	    		  var check = confirm('Bạn có muốn xóa ' + checked.length + ' slide item');
	    		  if (check == true) {
    		    	  $.ajax({
    		    		  type: "post",
    		    		  url: "<%=request.getContextPath()%>/admin/slide/deleteSelected",
    		    		  data: {checked: checked},
    		    		  cache: false,
    		    		  success: function(data){
    		    		    alert('Bạn vừa xóa' + data + ' slide');
    	  ...
    	  activeSlide = function(id, active) {
    		/* console.log(id);  
    		console.log(active); */
			$.ajax({
				url: '${pageContext.request.contextPath }/admin/slide/activeSlide',
				type: 'POST',
				cache: false,
				data: {
					id: id,
				...
				success: function(response){
					if (response == 1) {
						$('#activeSlide-' + id).toggleClass('active');
						active = active == 1 ? 0 : 1;
						$('#activeSlide-' + id).attr("onclick","activeSlide(" + id + "," + active + ")");
					}	
				},
				error: function (){
